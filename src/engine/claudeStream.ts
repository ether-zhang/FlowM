...
/**
 * Pull the session id out of the `init` event, so the canvas engine can `--resume` the same
 * Claude Code session on later turns (keeping the static guide + history in-session). Returns
 * null for every other line.
 */
export function extractSessionId(line: string): string | null {
  try {
    const ev = JSON.parse(line)
    if (ev && ev.type === 'system' && ev.subtype === 'init' && typeof ev.session_id === 'string') return ev.session_id
  } catch {
    // non-JSON noise
  }
  return null
