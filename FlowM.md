...
- 人机交互相关
    ...
    - 布局优化
        ...
        - [ ] ②+67·内容驱动的尺寸 + 间距节律归一（进行中）：先按文本定盒（`labelBoxSize`：行数×行高、最长行宽，CJK≈字号 / ASCII≈0.6×，**只增不减**，菱形/椭圆额外放大），再 `normalizeSpacing` **逐边沿箭头方向**把边到边缝归一到目标缝——用真实盒厚、**任意方向通用**、近轴方向吸附对齐、回边经 DFS 排除、仅动本批 movable、源锚定。排在 `resolveOverlaps` 前；纯函数 `layout.ts`，可单测。（合并原 67「按文本定盒长宽」与 ②「间距归一」）
